...
namespace FuncMatic
{
    public partial class Main : Form
    {
        ...
        private void startsave()
        {
            ...
            string isolatedStorageFileName = "settings.xml"; // The desired filename in isolated storage

            // Save the XML file from the base directory to isolated storage (only if it doesn't exist)
        ...
        {
            // Check if Ctrl key and 'A' key are both pressed
        ...
        {
            // Set the unsaved changes flag when the TextBox content changes.
        ...
        private void ColorWordsBeforeParenthesis()
        {
            ...
            // Define a regular expression pattern to match words before an opening parenthesis
            string pattern = @"(\w+)\(";
            ...
            foreach (Match match in matches)
            {
                ...
                // Check if the word is not preceded by a letter or digit
                ...
                {
                    // Select and color everything inside the parentheses
            ...
            // Define a list of operators to color
        ...
        private void Main_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (savefilecontents != null)
            {
                ...
                {
                    // Ask the user if they want to save changes before closing.f
                    ...
                    if (result == DialogResult.Yes)
                    {
                        if (savepath != null)
                        {
                            savefile_Click(sender, e); // Save changes if the user chooses "Yes."
                        }
                        else
                        {
                    ...
                    {
                        e.Cancel = true; // Cancel the form closing if the user chooses "Cancel."
                    }
                }
            }
            else
        ...
        {
            unit unitForm = new unit(this); // Pass the reference to the Main form
            unitForm.StartPosition = FormStartPosition.CenterParent; // Center Form3 on its owner
            unitForm.Owner = this; // Set Form2 as the owner of Form3
            unitForm.ShowDialog();
        }
        ...
        {
            // Create an OpenFileDialog instance to let the user choose the file to open.
        ...
        private void panel3_Paint(object sender, PaintEventArgs e)
        {
            ...
            LinearGradientBrush gradientBrush = new LinearGradientBrush(
                        gradientRect,
                                                  Color.FromArgb(245, 246, 248), // Start color
        Color.FromArgb(192, 211, 226), // End color
                        LinearGradientMode.Vertical); // Horizontal gradient
        ...
        private void panel8_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (panel.Width > 0 && panel.Height > 0)
            {
                ...
                LinearGradientBrush gradientBrush = new LinearGradientBrush(
                    gradientRect,
                    Color.FromArgb(248, 248, 248), // Start color (e.g., red)
                    Color.FromArgb(205, 207, 209), // End color (e.g., blue)
        ...
        private void panel6_Paint(object sender, PaintEventArgs e)
        {
            ...
            // Create a rounded rectangle path with a border radius of 4 pixels
            int borderRadius = 4; // Adjusted border radius
            Rectangle gradientRect = new Rectangle(0, 0, button.Width, button.Height);
            GraphicsPath path = new GraphicsPath();
            path.AddArc(gradientRect.Left, gradientRect.Top, borderRadius * 2, borderRadius * 2, 180, 90); // Top-left corner
            path.AddArc(gradientRect.Right - borderRadius * 2, gradientRect.Top, borderRadius * 2, borderRadius * 2, 270, 90); // Top-right corner
            path.AddArc(gradientRect.Right - borderRadius * 2, gradientRect.Bottom - borderRadius * 2, borderRadius * 2, borderRadius * 2, 0, 90); // Bottom-right corner
            path.AddArc(gradientRect.Left, gradientRect.Bottom - borderRadius * 2, borderRadius * 2, borderRadius * 2, 90, 90); // Bottom-left corner
            path.CloseFigure();

            // Create a region using the rounded rectangle path
            Region region = new Region(path);

            // Set the button's region to the rounded rectangle region
        ...
        {
            // Define your logic for resizing and repositioning controls here
            // For example, you can adjust the size and location of controls based on the new size of the UserControl.
            int bigw = this.Width;
            int newWidth = panel8.Width;
            int newHeight = panel8.Height;
            int xpoint = (((int)newWidth - panel9.Width) / 2);
        ...
        private void opensettings_Click(object sender, EventArgs e)
        {
            ...
            settings.StartPosition = FormStartPosition.CenterParent; // Center Form3 on its owner
            settings.Owner = this; // Set Form2 as the owner of Form3
            settings.ShowDialog();
        }
        private void simpleopen_Paint(object sender, PaintEventArgs e)
        {
            ...
            // Create a linear gradient brush for the button's background
            LinearGradientBrush gradientBrush = new LinearGradientBrush(
                gradientRect,
                Color.FromArgb(248, 248, 248), // Start color (e.g., red)
                Color.FromArgb(205, 207, 209), // End color (e.g., blue)
            ...
            // Fill the button's background with the gradient
        ...
        {
            string word = ans; // The word you want to color as "ans"
            Color color = Color.FromArgb(110, 165, 238);
            string text = funcboard.Text;

            // Use a regular expression to find "ans" as a whole word
        ...
        private void Calculate()
        {
            try
            {
                ...
                // Define a regular expression pattern to match valid polynomial expressions
                // This pattern checks for terms like "ax^b" or "ax^by^c" where a, b, and c can be numbers
                ...
                foreach (var variable in rvariables)
                {
                    ...
                    // Use a regular expression to match variableName as a standalone word
                    string patternv = $@"\b{Regex.Escape(variableName)}\b";

                    // Replace all occurrences of variableName with variableValue in the expression
                ...
                {
                    // Handle expressions in parentheses and exponentiation
                        ...
                        if (oldfunc != null)
                        {
                            ...
                            try
                            {
                                answer = Regex.Replace(answer, @"(\d+(\.\d+)?)\s*\^\s*(-?\d+(\.\d+)?)", match =>
                                {
                                    ...
                                    {
                                        // Handle parsing error for baseNumber here
                            ...
                            {
                                // Handle any potential exceptions that may occur during evaluation.
                ...
                foreach (string function in predefinedFunctions)
                {
                    if (expression.Contains($"{function}("))
                    {
                        ...
                        if (startIndex >= 0 && endIndex > startIndex)
                        {
                            ...
                            // Replace the sqrt(...) expression with the calculated square root
